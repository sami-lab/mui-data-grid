...
exports.update = catchAsync(async (req, res, next) => {
  const { agentToken, authToken, apiKey, agentYaml, agentAdd } = req.body;

  const doc = await Agent.findByIdAndUpdate(
    req.params.id,
    { agentToken, authToken, apiKey, agentYaml, agentAdd },
    {
      new: true,
      runValidators: true,
    }
...
exports.createOne = catchAsync(async (req, res, next) => {
  const { id, agentToken, authToken, apiKey, agentYaml, agentAdd } = req.body;
...
exports.getAll = catchAsync(async (req, res, next) => {
  const doc = await Agent.find().sort({ createdOn: "descending" });
